...
**Relates to:** the agreed joining-service architecture, **Option B*** (#165) — Sacha's Path B with an `auth_methods` swap to `email_code` + `delegated_verification`.
...
| The warm invite: admit-on-arrival | scaffold + content |

Green has two origins. One is earned over time — the reengagement funnel above, where a declined "we don't know you yet" becomes a vouch, an event, a shared context, and the next application arrives corroborated. The other is issued ahead: an admin who already holds that relationship can hand someone green at the door, before they ever fill in a form. This is the warm invite, and it is the proactive twin of reengagement — the same soft membrane, run forwards instead of backwards.

**Issuing the invite is the admit decision, made early.** The cold path is: a person arrives unknown, an admin reads the application in full, and — if they choose — commits a signed admit that cites its corroboration (Fairness by construction). The warm path moves that act to the front. When an admin invites someone they know, the invitation is that signed admit, exercised in advance; its cited corroboration is the relationship the admin is vouching for. So when the invited person later appears, generates their agent key, and presents the invite, R&O vouches for that specific agent automatically — there is no second decision waiting at the door, because the decision was already made and recorded when the invite was issued. The membrane is not bypassed. The human relational judgement the soft membrane exists to require still happens; it simply happens earlier.

This carries a governance consequence worth stating plainly: **issuing an invitation is exercising admission authority.** It has the same weight as a green admit in the queue and should feel like one — a deliberate, attributable act, not a casual share. Principle 2 (friction matches consequence) still holds: an admit is light-touch, but it is a considered touch, and the warm invite is that same considered touch moved forward in time. The invite-issuance surface is a decision surface.

**The invitation, as a primitive.** A pending person has no agent key at invite time (_Data & ownership_), so an invitation is bound to the only stable identifier available — an email — and is single-use, with an optional expiry (a standing admit-minting token is a small liability; let unused invites lapse). One invitation is one per-person vouch. A shareable or bulk link is a categorically weaker thing — "a door was opened," not "this person is known" — and does not mint green; if such a thing is ever wanted, it is a separate primitive with its own lesser standing, never this one wearing the same name.

**How the corroboration reaches the door.** The invitation reference does not need to travel through the joining service to be trustworthy. The membrane proof an agent commits at genesis carries only the agent key, the DNA hash, a timestamp and a nonce — there is no slot for an application-level reference, and the partner-side tracking id is audit-only. The linkage is held on R&O's side instead, and falls out of the Option B* model: because R&O is the party that vouches for the agent, R&O has the agent key in hand at the moment it issues that vouch, and binds it to the invitation then. The corroboration a warm green admit cites is exactly this — _invitation #X, redeemed by agent_ ‹key› — captured by R&O, never dependent on a third party echoing it back. One constraint keeps this intact: R&O must vouch for the specific agent in-session, in the moment the person joins; it must not pre-issue a detached "this email is cleared" token before the agent key exists, which would both lose the linkage and go stale against the verification freshness window. The model that preserves the audit trail is the one the mechanism already wants.

5. **Seeing whether an invite landed** — the invited ledger. Because R&O holds _invitation ↔ agent key_, it can tell whether an invitation was taken up. When the invited person completes their join and creates their R&O profile, that profile is authored by the same agent key (authorship lives in the action header). R&O matches the new profile's author against the outstanding invitations, and the invitation moves from invited to onboarded. An admin reads this in **an invited ledger** — not a queue, because nothing there is awaiting a decision; the decision was made at issuance. The ledger shows what is outstanding, what was taken up, and what lapsed unused, and the onboarded rows persist as a provenance record: an onboarded row is the cited corroboration behind that member's green admit. This is admin-facing operational tooling, kept distinct from the membership-facing mirror — the mirror reports only the aggregate shape, and "sources of green" in it now legibly includes admin invite alongside vouch and shared context.

This sits cleanly under the principles. Agent-centric (6): the agent-key binding is part of R&O's record of its own vouch, not custody of the invited person's data. Accountable to the membership (7): warm admits appear in the mirror as a source of green and as a pattern an admin can over- or under-use, auditable in aggregate without exposing who was invited. Describe against the criterion (3): a warm admit's cited corroboration — invitation #X — is factual, auditable, and fine for the applicant to read.
...
- The joining flow is **Option B\*** — Sacha's Path B with an `auth_methods` swap to `email_code` + `delegated_verification` — not `hc_auth_approval`. Admission is an admin approval in R&O's own app plus a per-agent vouch; pending applicants have no pubkey at triage.
...
1. **Connection corroboration source.** Green-as-connection is the keystone, and a green admit must cite that corroboration to commit (see _What the admin sees_). The invite path is now specified — see _The warm invite: admit-on-arrival_ — and resolves to a concrete, R&O-side mechanism: admit-on-arrival, with the invitation ↔ agent-key linkage held by R&O at vouch time, so the cited corroboration never depends on a third party echoing a reference back. What remains: the member-vouch path as a second green source (same shape — corroboration the community side confirms — not yet designed), and the implementation of the invitation primitive and invited ledger in R&O (scoped in #168). Until those land, green still ships as "aligned" and earns "connected" as each source is wired.
