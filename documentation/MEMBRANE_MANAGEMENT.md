...
**Version:** 0.1.0 · **Status:** design, not built — proposed for community discussion and agreement before implementation.
...
**Relates to:** the R&O joining-service architecture (#125) and the agreed joining flow, **Option B\*** (#165) — Sacha's Path B with an `auth_methods` swap to `email_code` + `delegated_verification`.
